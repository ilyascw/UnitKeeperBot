...
from unitkeeper_backend.application.jobs.notifications import ReminderPublisher, SprintMemberReport, SprintReportPublisher
...
async def test_sprint_reports_are_durable_deduplicated_and_correlated() -> None:
    ...
    args = dict(group_id=7, group_name="Home", owner_user_id=1, period="2026-07-06..2026-07-12", planned_units="10", completed_units="12", reports=[SprintMemberReport(1, "5", "6", "+1"), SprintMemberReport(2, "5", "6", "+1")], correlation_id="close-7")
...
async def test_reminders_skip_empty_queue_and_are_idempotent() -> None:
    ...
    await job.pending_approvals(group_id=7, owner_user_id=1, count=0, period="2026-07-12", correlation_id="reminders-1")
    await job.pending_approvals(group_id=7, owner_user_id=1, count=2, period="2026-07-12", correlation_id="reminders-1")
    await job.pending_approvals(group_id=7, owner_user_id=1, count=2, period="2026-07-12", correlation_id="reminders-1")
    await job.sprint_deadline(group_id=7, user_ids=[1, 2], deadline="today", period="2026-07-12", correlation_id="reminders-1")
    ...
    class Closer:
        ...
        async def close_due_sprint(self, *, group_id: int, correlation_id: str) -> ClosedSprint | None:
            if group_id == 7 and not self.closed:
                self.closed = True
                return ClosedSprint(7, "Home", 1, "2026-07-06..2026-07-12", "10", "12", [SprintMemberReport(1, "10", "12", "+2")])
            return None
