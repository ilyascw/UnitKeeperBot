...
## Source Of Truth
- Until migration is complete, `UnitKeeperBot/` is the source of truth for legacy behavior. **This directory is gitignored and not on GitHub** — it only exists on local dev machines that cloned it separately. If you don't have it locally, ask for it rather than assuming it's missing from the project.
  - The legacy bot's own history is still available from `origin/main` (it hasn't been touched or rewritten yet — see [Branching](#branching)): `git checkout origin/main -- .` into a scratch directory, or just browse `git log origin/main` / `git show origin/main:<path>`, if you need the old code and don't have `UnitKeeperBot/` checked out locally.
...
## Target Structure
- [`miniapp`](miniapp): native Telegram Mini App with Telegram UI Kit.
- [`common`](common): shared database layer for backend and bot, including SQLAlchemy models, Alembic migrations, and DB integration code.
- [`backend`](backend): FastAPI app with business use cases, API contracts, auth, schedulers, and integrations.
- [`bot`](bot): thin Telegram bot for notifications, deep links, and minimal fallback flows.
- [`docs`](docs): migration notes and legacy functional artifacts.
...
8. **Expose it over HTTPS:** `ngrok http 5173`. Copy the resulting `https://*.ngrok-free.dev` URL into `bot/.env`'s `UNITKEEPER_MINIAPP_URL`, then `docker compose up -d bot` to restart the bot with the new Web App URL.
...
- Each migrated feature should be traceable back to the legacy artifact in [`docs/legacy-functionality.md`](docs/legacy-functionality.md).
...
- The virtual environment must live inside the working service directory, not at repo root. Example: when working on [`backend`](backend), create and activate `backend/.venv`.
